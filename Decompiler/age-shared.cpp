...
// Use of "deprecated" _open function
...
Instruction_Definition definitions[] = {
...
int open_or_die(const std::filesystem::path& file_path, int flag, int mode) {
	int ret = _open(file_path.string().c_str(), flag, mode);
...
Instruction_Definition* instruction_for_op_code(unsigned int op_code) {
	int low = 0;
	int high = size_of_array(definitions) - 1;

	while (low <= high) {
		int mid = (low + high) >> 1;
		unsigned int mid_op_code = definitions[mid].op_code;
...
Instruction_Definition* instruction_for_label(std::string label) {
	...
	for (unsigned int i = 0; i < size_of_array(definitions); i++) {
		if (label == definitions[i].label) {
